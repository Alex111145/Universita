
public class ThreadIterruptExit {
	public static void main(String args[]) {
		System.err.println("I thread stanno per partire");
		Thread t1 = new ExampleWithSleeps3("t1"); t1.start();
		Thread t2 = new ExampleWithSleeps3("t2"); t2.start();
		Thread t3 = new ExampleWithSleeps3("t3"); t3.start();
		System.err.println("I thread sono partiti\n");
		t1.interrupt();
		t2.interrupt();
		t3.interrupt();		
		try {
			t1.join();
			t2.join();
			t3.join();
		} catch (InterruptedException e) { }
		System.err.println("chiude l'applicazione");
		System.exit(0); //chiude l'applicazione in ogni caso
	}
}
