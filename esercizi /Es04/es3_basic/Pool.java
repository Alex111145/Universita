...
public class Pool {
	...
	State[] philStates;
	int numSticks;  // numero stick disponibili
	Pool(int np, int ns) {
		philStates=new State[np];
		numSticks=ns;
		for(int i=0; i<Table.NUM_PHIL; i++) {
			philStates[i]=State.Idle; // inizializza lo stato dei filosofi
		}
	}
	...
	public synchronized void takeTwo(int philId) {
		...
		notifyAll();  // per svegliare waiter
	...
	public synchronized void scheduleNext() {
		...
		if(idP==-1) {
			...
			numSticks-=2;
			notifyAll();  // per svegliare filosofi
		}
	}
}
